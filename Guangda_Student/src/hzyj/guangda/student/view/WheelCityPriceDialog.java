...
import hzyj.guangda.student.view.WheelCityDialog.OnComfirmClickListener;
import kankan.wheel.widget.WheelView;
import kankan.wheel.widget.adapters.ArrayWheelAdapter;


public class WheelCityPriceDialog extends BaseDialog{
	
	private WheelView mcitywheel;
	ArrayList<OpenCity> cityInfor;
	private  String[] ArrayCity;
	private OnCityClickListener mOnComfirmClickListener;

	public WheelCityPriceDialog(Context context) {
		super(context, R.style.dim_dialog);
		// TODO Auto-generated constructor stub
	}
	
	public void WheelCityData( ArrayList<OpenCity> cityInfor) {
		this.cityInfor=cityInfor;
	}

	public WheelCityPriceDialog(Context context, boolean cancelable, OnCancelListener cancelListener) {
		super(context, cancelable, cancelListener);
	}
	
	public WheelCityPriceDialog(Context context, int theme) {
		super(context, theme);
	}
...
//				}
				...
				String city = cityInfor.get(mcitywheel.getCurrentItem()).getCityname();
				String cityid=cityInfor.get(mcitywheel.getCurrentItem()).getCityid();
				
				if (mOnComfirmClickListener != null) {
					mOnComfirmClickListener.onCityBtnClick(city,cityid);
				}
				dismiss();
			}
		});
	...
	public interface OnCityClickListener {
		void onCityBtnClick(String city,String cityid);
	}
	...
	private void getcity(){
		ArrayCity=new  String[cityInfor.size()];
		for(int i=0;i<cityInfor.size();i++){
			ArrayCity[i]=cityInfor.get(i).getCityname();
    	}
	}
