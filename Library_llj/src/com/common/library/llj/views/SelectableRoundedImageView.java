...
public class SelectableRoundedImageView extends ImageView {
	...
	private static final ScaleType[] sScaleTypeArray = { ScaleType.MATRIX, ScaleType.FIT_XY, ScaleType.FIT_START, ScaleType.FIT_CENTER, ScaleType.FIT_END, ScaleType.CENTER, ScaleType.CENTER_CROP,
	...
	public SelectableRoundedImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		mLeftTopCornerRadius = a.getDimensionPixelSize(R.styleable.SelectableRoundedImageView_sriv_left_top_corner_radius, 0);
		mRightTopCornerRadius = a.getDimensionPixelSize(R.styleable.SelectableRoundedImageView_sriv_right_top_corner_radius, 0);
		mLeftBottomCornerRadius = a.getDimensionPixelSize(R.styleable.SelectableRoundedImageView_sriv_left_bottom_corner_radius, 0);
		mRightBottomCornerRadius = a.getDimensionPixelSize(R.styleable.SelectableRoundedImageView_sriv_right_bottom_corner_radius, 0);

		if (mLeftTopCornerRadius < 0.0f || mRightTopCornerRadius < 0.0f || mLeftBottomCornerRadius < 0.0f || mRightBottomCornerRadius < 0.0f) {
			throw new IllegalArgumentException("radius values cannot be negative.");
		}

		mRadii = new float[] { mLeftTopCornerRadius, mLeftTopCornerRadius, mRightTopCornerRadius, mRightTopCornerRadius, mRightBottomCornerRadius, mRightBottomCornerRadius, mLeftBottomCornerRadius,
	...
	static class SelectableRoundedCornerDrawable extends Drawable {
		...
		private void adjustCanvasForBorder(Canvas canvas) {
			...
			final float newScaleX = mBounds.width() / (mBounds.width() + mBorderWidth + mBorderWidth);
			final float newScaleY = mBounds.height() / (mBounds.height() + mBorderWidth + mBorderWidth);

			canvas.scale(newScaleX, newScaleY);
			if (ScaleType.FIT_START == mScaleType || ScaleType.FIT_END == mScaleType || ScaleType.FIT_XY == mScaleType || ScaleType.FIT_CENTER == mScaleType || ScaleType.CENTER_INSIDE == mScaleType
		 ...
		 * 
		 * @param colors
		 *            The desired border color. If it's null, no border will be drawn.
		 * 
		 */
		public void setBorderColor(ColorStateList colors) {
			if (colors == null) {
