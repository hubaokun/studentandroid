...
/**
 * A singleton to present a simple static interface for building requests with {@link BitmapRequestBuilder} and maintaining an {@link Engine}, {@link BitmapPool},
...
public class Glide {
	...
	private static final String DEFAULT_DISK_CACHE_DIR = "glide_disk_cache";
	...
	public static File getPhotoCacheDir(Context context, String cacheName) {
		...
		if (BaseApplication.PIC_PATH != null) {
			// File result = new File(BaseApplication.PIC_PATH, cacheName);
			File result = new File(BaseApplication.PIC_PATH);
			...
			throw new IllegalArgumentException("default disk cache dir is null");
		}
	 ...
	 *             if the Glide singleton has already been created.
	 */
	@Deprecated
	...
	/**
	 * Returns the {@link com.bumptech.glide.load.engine.bitmap_recycle.BitmapPool} used to temporarily store {@link android.graphics.Bitmap}s so they can be reused to avoid garbage collections.
	 * 
	 * <p>
	 * Note - Using this pool directly can lead to undefined behavior and strange drawing errors. Any {@link android.graphics.Bitmap} added to the pool must not be currently in use in any other part
	 * of the application. Any {@link android.graphics.Bitmap} added to the pool must be removed from the pool before it is added a second time.
	 * </p>
	 * 
	 * <p>
	 * Note - To make effective use of the pool, any {@link android.graphics.Bitmap} removed from the pool must eventually be re-added. Otherwise the pool will eventually empty and will not serve any
	 * useful purpose.
	 * </p>
	 * 
	 * <p>
	 * The primary reason this object is exposed is for use in custom {@link com.bumptech.glide.load.ResourceDecoder}s and {@link com.bumptech.glide.load.Transformation}s. Use outside of these classes
	 ...
	 * 
	 * <p>
	 * Enough Bitmaps are added to completely fill the pool, so most or all of the Bitmaps currently in the pool will be evicted. Bitmaps are allocated according to the weights of the given sizes,
	 * where each size gets (weight / prefillWeightSum) percent of the pool to fill.
	 * </p>
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * This method should be used with caution, overly aggressive pre-filling is substantially worse than not pre-filling at all. Pre-filling should only be started in onCreate to avoid constantly
	 * clearing and re-filling the {@link com.bumptech.glide.load.engine.bitmap_recycle.BitmapPool}. Rotation should be carefully considered as well. It may be worth calling this method only when no
	 * saved instance state exists so that pre-filling only happens when the Activity is first created, rather than on every rotation.
	 * </p>
	 * 
	 * @param bitmapAttributeBuilders
	 *            The list of {@link com.bumptech.glide.load.engine.prefill.PreFillType.Builder Builders} representing individual sizes and configurations of {@link android.graphics.Bitmap}s to be
	 *            pre-filled.
	 */
	 ...
	 * 
	 * <p>
	 * The default {@link MemoryCategory} is {@link MemoryCategory#NORMAL}. {@link MemoryCategory#HIGH} increases Glide's maximum memory usage by up to 50% and {@link MemoryCategory#LOW} decreases
	 * Glide's maximum memory usage by 50%. This method should be used to temporarily increase or decrease memory useage for a single Activity or part of the app. Use
	 * {@link GlideBuilder#setMemoryCache(MemoryCache)} to set a permanent memory size if you want to change the default.
	 * </p>
	 */
	...
	/**
	 * Cancel any pending loads Glide may have for the target and free any resources (such as {@link Bitmap}s) that may have been loaded for the target so they may be reused.
	 * 
	 * @param target
	 ...
	 * @throws IllegalArgumentException
	 *             if an object other than Glide's metadata is set as the view's tag.
	 */
	...
	/**
	 * Use the given factory to build a {@link ModelLoader} for models of the given class. Generally the best use of this method is to replace one of the default factories or add an implementation for
	 * other similar low level models. Typically the {@link RequestManager#using(com.bumptech.glide.load.model.stream.StreamModelLoader)} or
	 * {@link RequestManager#using(com.bumptech.glide.load.model.file_descriptor.FileDescriptorModelLoader)} syntax is preferred because it directly links the model with the ModelLoader being used to
	 * load it. Any factory replaced by the given factory will have its {@link ModelLoaderFactory#teardown()} method called.
	 * 
	 * <p>
	 ...
	 *            The model class.
	 * @param resourceClass
	 *            The resource class the model loader will translate the model type into.
	 * @param factory
	 *            The factory to use.
	 * @param <T>
	...
	/**
	 * Removes any {@link ModelLoaderFactory} registered for the given model and resource classes if one exists. If a {@link ModelLoaderFactory} is removed, its {@link ModelLoaderFactory#teardown()}
	 * method will be called.
	 * 
	 * @deprecated Use {@link #register(Class, Class, com.bumptech.glide.load.model.ModelLoaderFactory)} to replace a registered loader rather than simply removing it.
	 * @param modelClass
	 *            The model class.
	 * @param resourceClass
	 *            The resource class.
	 ...
	 * 
	 * @param modelClass
	 *            The class to get a {@link ModelLoader} for.
	 * @param resourceClass
	 *            The resource class to get a {@link ModelLoader} for.
	 * @param context
	 *            Any context.
	 * @param <T>
	 *            The type of the model.
	 ...
	 * 
	 * @param model
	 *            A non null model object whose class we will get a {@link ModelLoader} for.
	 * @param resourceClass
	 *            The resource class to get a {@link ModelLoader} for.
	 * @param context
	 *            Any context.
	 ...
	 * 
	 * <p>
	 * Any requests started using a context will only have the application level options applied and will not be started or stopped based on lifecycle events. In general, loads should be started at
	 * the level the result will be used in. If the resource will be used in a view in a child fragment, the load should be started with {@link #with(android.app.Fragment)} using that child fragment.
	 * Similarly, if the resource will be used in a view in the parent fragment, the load should be started with {@link #with(android.app.Fragment)} using the parent fragment. In the same vein, if the
	 * resource will be used in a view in an activity, the load should be started with {@link #with(android.app.Activity)} .
	 * </p>
	 * 
	 * <p>
	...
	/**
	 * Begin a load with Glide that will be tied to the given {@link android.app.Activity}'s lifecycle and that uses the given {@link Activity}'s default options.
	 * 
	 * @param activity
	 *            The activity to use.
	...
	/**
	 * Begin a load with Glide that will tied to the give {@link android.support.v4.app.FragmentActivity}'s lifecycle and that uses the given {@link android.support.v4.app.FragmentActivity}'s default
	 * options.
	 * 
	 * @param activity
	 *            The activity to use.
	...
	/**
	 * Begin a load with Glide that will be tied to the given {@link android.support.v4.app.Fragment}'s lifecycle and that uses the given {@link android.support.v4.app.Fragment}'s default options.
	 * 
	 * @param fragment
	 *            The fragment to use.
	 * @return A RequestManager for the given Fragment that can be used to start a load.
